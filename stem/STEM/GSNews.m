...
-(GSNews*) initWithFrame:(CGRect)frame gameSceneManager:(GameSceneManager*)manager {
    
    NSLog(@"GSNews initWithFrame:andManager");
    
    if (self = [super initWithFrame:frame gameSceneManager:manager]) {
        //do initializations here.
        
        [self addSubview: manager.webViewNews];
        manager.webViewNews.translatesAutoresizingMaskIntoConstraints = NO;
...
- (void)didMoveToSuperview {
    ...
    if(self.superview)
    {
        NSLog(@"GSNews didMoveToSuperview: Added to superview");
        ...
        m_pManager.webViewNews.scrollView.backgroundColor = [UIColor colorWithRed:0.58 green:0.91 blue:0.83 alpha:1.0];
        //        m_pManager.webViewNews.scrollView.backgroundColor = [UIColor colorWithRed:255.0 green:255.0 blue:255.0 alpha:1.0];
        
        m_pManager.webViewNews.delegate = self;
        ...
        [m_pManager.webViewNews addSubview:gradientTopImgView];
        ...
        [m_pManager.webViewNews addSubview:gradientBotImgView];
        
        [self initializeButtons];
        
    } else {
        
        NSLog(@"GSNews didMoveToSuperview: Removed from superview");
        
        [self removeConstraints:self.constraints];
        [m_pManager.webViewNews removeConstraints:m_pManager.webViewNews.constraints];
...
-(void)webView:(UIWebView *) webView didFailLoadWithError:(NSError *)error {
    NSLog(@"GSNews:webView didFailLoadWithError");
}

-(void)webViewDidStartLoad:(UIWebView *)webView {
    NSLog(@"GSNews:webViewDidStartLoad");
}

- (void)webViewDidFinishLoad:(UIWebView *)webView {
    
    NSLog(@"--------GSNews webViewDidFinishLoad");
...
/*
 ...
 for (UIView *view in m_pManager.webViewNews.scrollView.subviews) {
...
- (void)layoutSubviews {
    NSLog(@"GSNews:layoutSubviews");
    
    [super layoutSubviews];
    //    CGFloat contentHeight = m_pManager.webViewNews.scrollView.contentSize.height;
    //    NSLog(@"--------contentHeight %+.6f\n", contentHeight);
    
    // Use JS to obtain the content height, because m_pManager.webViewNews.scrollView.contentSize.height, doesn't have the correct size
    // when layoutSubviews is first called
    NSString *height = [m_pManager.webViewNews stringByEvaluatingJavaScriptFromString:@"document.body.scrollHeight;"];
...
-(void)initializeButtons {
    ...
    [m_pManager.webViewNews.scrollView addSubview: btnAlarm];
    //    [m_pManager.webViewNews.scrollView addSubview: btnShare];
}
...
- (void) update:(double)delta {
    ...
    if(_autoScroll) {
        ...
        CGPoint pt = CGPointMake(0, m_pManager.webViewNews.scrollView.contentOffset.y);
        pt.y += 1;
        if(m_pManager.webViewNews.scrollView.contentSize.height > pt.y + m_pManager.webViewNews.frame.size.height) {
            
            //          NSLog(@"contentOffset.y %+.6f\n", pt.y);
            [m_pManager.webViewNews.scrollView setContentOffset:CGPointMake(0, pt.y) animated: NO];
        } else {
            _autoScroll = NO;
...
- (void)initWebViewConstraints {
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:m_pManager.webViewNews
...
- (void)updateButtonConstraints {
    ...
    [m_pManager.webViewNews addConstraint:[NSLayoutConstraint constraintWithItem:btnAlarm
                                                                   attribute:NSLayoutAttributeCenterY
                                                                   relatedBy:NSLayoutRelationEqual
                                                                      toItem:m_pManager.webViewNews.scrollView
    ...
    /*
     [m_pManager.webViewNews addConstraint:[NSLayoutConstraint constraintWithItem:btnShare
     attribute:NSLayoutAttributeCenterY
     relatedBy:NSLayoutRelationEqual
     toItem:m_pManager.webViewNews.scrollView
     attribute:NSLayoutAttributeTop
    ...
    [m_pManager.webViewNews addConstraint:[NSLayoutConstraint constraintWithItem:gradientTopImgView
                                                                   attribute:NSLayoutAttributeWidth
                                                                   relatedBy:NSLayoutRelationEqual
                                                                      toItem:m_pManager.webViewNews.scrollView
                                                                   attribute:NSLayoutAttributeWidth
    ...
    [m_pManager.webViewNews addConstraint:[NSLayoutConstraint constraintWithItem:gradientBotImgView
                                                                   attribute:NSLayoutAttributeWidth
                                                                   relatedBy:NSLayoutRelationEqual
                                                                      toItem:m_pManager.webViewNews.scrollView
                                                                   attribute:NSLayoutAttributeWidth
