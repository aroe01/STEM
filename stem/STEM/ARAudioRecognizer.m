...
- (void)levelTimerCallback:(NSTimer *)timer
{
	...
	if (self.lowPassResults > 0.75 && self.delegate && [self.delegate respondsToSelector:@selector(audioRecognized:)]) {
        [self.delegate audioRecognized:self];

        NSLog(@"ARAudioRecognizer:audioRecognized %f", _lowPassResults);
        _lowPassResults = 0.0f;
